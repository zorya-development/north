...
└── crates/
    ...
    │       ├── app_store.rs    # AppStore { tasks, projects, tags, saved_filters, task_detail_modal, task_create_modal, filter_dsl, status_bar, modal }
    ...
    │       ├── task_create_modal_store.rs # TaskCreateModalStore: creation modal state, defaults, lifecycle
    ...
    │       │   └── traversable_task_list/ # Container/controller/view (keyboard-driven tree list with inline edit/create + drag-and-drop)
    │       └── components/
...
- **`dto`** — Pure data types with serde + chrono, no IO. Compiled for both server and WASM. Key types: `Task` (includes enrichment fields: project_title, tags, subtask_count, completed_subtask_count, actionable), `CreateTask` (includes optional `sort_key` for positional insertion), `TaskFilter` (complex query object), `ProjectFilter`, `ProjectStatus` (Active, Archived), `UserSettings` (review_interval_days, default_sequential_limit), `SavedFilter`, `DslSuggestion` (autocomplete suggestion DTO with label, value, color, start position). Utility modules: `serde_helpers` (three-state Option serialization), `sort_key` (fractional indexing for item ordering).
...
- **`stores`** — Reactive client state (`north-stores`). `AppStore` wraps `TaskStore` + `ProjectStore` + `TagStore` + `SavedFilterStore` + `TaskDetailModalStore` + `TaskCreateModalStore` + `FilterDslStore` + `StatusBarStore` + `ModalStore`, provided globally via context. `TaskStore` holds tasks in `RwSignal<Vec<Task>>` — pages create filtered `Memo`s over the shared data. Supports optimistic updates (immediate UI, async sync). `FilterDslStore` manages DSL query text, async server-side validation, autocomplete suggestions, and execution results — the filter page controller delegates all DSL state to this store. `ModalStore` is a string-based modal registry (`open()`, `close()`, `is_open()`, `is_any_open()`) that decouples keyboard logic from DOM structure. `StatusBarStore` manages bottom-bar messages with info/danger variants. `TaskCreateModalStore` manages creation modal state, defaults (project_id, parent_id), and lifecycle. Individual stores (`TagStore`, `SavedFilterStore`) cache domain data for pickers and navigation.
...
- **`app`** — Leptos library crate. Features: `hydrate` (WASM client), `ssr` (server-side, pulls in north-core/north-server-fns/argon2/jsonwebtoken). Pages follow container/controller/view pattern and interact with stores for data. Complex stateful domain components live in `containers/` (pickers, sidebar, autocomplete, task checkbox, task list, traversable task list, task list item, inline task input, create modal, detail modal). Simpler/presentational components live in `components/` (date picker, layout, filter autocomplete, keybindings modal, status bar, drag_drop, task_meta, theme toggle). **`atoms/`** contains UI Kit atoms — currently `Text` (TextVariant, TextColor, TextTag) based on Material Design 3 type scale. Atoms use `enum.classes() -> &'static str` pattern for variant/color/size mapping. Prefer `<Text>` over raw Tailwind for typography.
...
- **TaskCreateModalStore:** Manages task creation modal state, default project_id/parent_id, and lifecycle. Methods: `open(project_id, parent_id)`, `close()`, `create_task(input)`. Wraps `ModalStore` and `TaskStore` for coordinated behavior. Provided via `AppStore`.
- **ModalStore:** Centralized string-based modal registry (`stores/modal_store.rs`). Methods: `open(name)`, `close(name)`, `is_open(name)`, `is_any_open()`. Replaces DOM inspection with reactive signals — decouples keyboard listeners (e.g. TraversableTaskList) from modal DOM structure. Used by TaskDetailModal, TaskCreateModal, and TraversableTaskList (which skips keyboard handling when any modal is open). Provided via `AppStore`, accessible via `use_modal_store()`.
...
- **Completed tasks toggle:** Pages using `TraversableTaskList` (Inbox) pass `show_completed: RwSignal<bool>` — the tree flattener includes completed tasks at the bottom of each parent group when toggled on. Other task list pages (today, all_tasks, project) pass an optional `completed_resource` to `TaskList`, where `CompletedSection` renders a toggle button with count and dimmed completed tasks below the active list.
- **TraversableTaskList:** Keyboard-driven tree list container (`containers/traversable_task_list/`). Builds a flat `Vec<FlatNode>` from hierarchical tasks via DFS pre-order (active sorted by sort_key first, completed at bottom per parent group). Maintains a task-ID-based cursor with tree-aware navigation (Up/Down = siblings, Left = parent, Right = first child). Supports inline editing (Enter to edit title, Enter to save, Escape to cancel) and inline creation (Ctrl+Enter after, Shift+Enter before, with Tab/Shift+Tab for indent/outdent). Supports both keyboard reordering (Shift+Arrow) and drag-and-drop — when `draggable=true`, provides `DragDropContext` and handles tree-aware drops (Above/Below/Nest zones) with cycle prevention via `is_descendant_of()`. Two keyboard modes: **global** (default, uses `window_event_listener`, skips when ModalStore reports any modal open) and **scoped** (element-level `on:keydown`, used inside modals like TaskDetailModal to avoid conflicts with parent keyboard listeners). Auto-selects first task on Tab focus into scoped TTL. Rendered via `<For each=flat_nodes key=|n| n.task_id>` for efficient keyed updates. Each item reuses `TaskListItem` with `hide_subtasks=true` (subtrees are flattened into the list instead). Pure tree utilities in `tree.rs` with unit tests. `CreateTask` DTO supports optional `sort_key` for positional insertion via `sort_key_between()`/`sort_key_after()`.
...
- **TaskListItem:** Reusable task row component (`containers/task_list_item/`). Renders checkbox, title, action bar (date/project/tag pickers, kebab menu), markdown body, metadata, and optional inline subtask list. Props: `hide_subtasks` (skip `InlineSubtaskList` rendering — used by `TraversableTaskList` which flattens subtrees into the parent list), `compact`, `draggable`, `show_project`, `show_review`, `depth`, `on_click`.
...
- **KeybindingsModal:** Help modal (`components/keybindings_modal.rs`) displaying keyboard shortcuts for TraversableTaskList navigation. Triggered by `?` key or button click. Shows shortcuts for cursor movement, tree navigation, inline editing, task creation, completion toggle, delete, and detail modal access.
