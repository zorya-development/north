...
└── crates/
    ...
    │       ├── app_store.rs    # AppStore { tasks, projects, tags, saved_filters, task_detail_modal, filter_dsl, status_bar, modal }
...
- **`stores`** — Reactive client state (`north-stores`). `AppStore` wraps `TaskStore` + `ProjectStore` + `TagStore` + `SavedFilterStore` + `TaskDetailModalStore` + `FilterDslStore` + `StatusBarStore` + `ModalStore`, provided globally via context. `TaskStore` holds tasks in `RwSignal<Vec<Task>>` — pages create filtered `Memo`s over the shared data. Supports optimistic updates (immediate UI, async sync). `FilterDslStore` manages DSL query text, async server-side validation, autocomplete suggestions, and execution results — the filter page controller delegates all DSL state to this store. `ModalStore` is a string-based modal registry (`open()`, `close()`, `is_open()`, `is_any_open()`) that decouples keyboard logic from DOM structure. `StatusBarStore` manages bottom-bar messages with info/danger variants. Individual stores (`TagStore`, `SavedFilterStore`) cache domain data for pickers and navigation.
