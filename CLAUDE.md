...
```
recurrence    (pure RRULE types — RecurrenceType, Frequency, Weekday, RecurrenceRule)
  ↑
dto           (pure data types, no IO — compiled for both server and WASM, re-exports recurrence)
  ↑
...
└── crates/
    ...
    │   └── src/
    │       └── lib.rs          # RecurrenceType, Frequency, Weekday, RecurrenceRule (parse/serialize RRULE strings, summarize)
    │
    ├── dto/                    # Pure data types (no IO, formerly "domain") — re-exports north-recurrence
    │   └── src/
    │       ├── lib.rs
    ...
    │       ├── sql_types.rs    # PG enum mappings (UserRoleMapping, ProjectViewTypeMapping, ProjectStatusMapping, RecurrenceTypeMapping)
    │       └── models/
    │           ├── mod.rs
    ...
    │       ├── app_store.rs    # AppStore { tasks, projects, tags, saved_filters, task_detail_modal, filter_dsl, status_bar, modal, browser_storage }
    │       ├── browser_storage_store.rs # BrowserStorageStore: reactive proxy over localStorage (lazy RwSignal per key)
    ...
    │       │   ├── task_checkbox/      # Container/controller/view (task completion checkbox with progress for parent tasks)
    │       │   ├── task_detail_modal/  # Container/view pattern (modal for task details, scoped TTL for subtasks)
    ...
    │           ├── recurrence_modal/   # Container/controller/view (recurrence rule editor; ReactiveRecurrenceRule wraps north-recurrence with Leptos signals)
    │           ├── keybindings_modal.rs # Keyboard shortcuts help modal (triggered by ? key)
    │           ├── status_bar.rs       # Bottom bar for contextual messages (info/danger with animated spinner)
...
- **`recurrence`** — Pure RRULE types and logic (`north-recurrence`). No IO, no framework deps — only `serde`. Types: `RecurrenceType` (Scheduled, AfterCompletion), `Frequency` (Daily, Weekly, Monthly, Yearly), `Weekday`, `RecurrenceRule` (structured type with `parse()`/`to_rrule_string()` for serialization, `summarize()` for human-readable display, `time_str()`/`set_time_str()` for time access). Re-exported by `north-dto` for backwards compatibility.
- **`dto`** — Pure data types with serde + chrono, no IO. Compiled for both server and WASM. Re-exports all types from `north-recurrence`. Key types: `Task` (includes enrichment fields: project_title, tags, subtask_count, completed_subtask_count, actionable, recurrence_type, recurrence_rule), `CreateTask` (includes optional `sort_key` for positional insertion), `TaskFilter` (complex query object), `ProjectFilter`, `ProjectStatus` (Active, Archived), `UserSettings` (review_interval_days, default_sequential_limit, timezone), `SavedFilter`, `DslSuggestion` (autocomplete suggestion DTO with label, value, color, start position). Utility modules: `serde_helpers` (three-state Option serialization), `sort_key` (fractional indexing for item ordering).
- **`db`** — Diesel infrastructure: `schema.rs` (auto-generated by `diesel print-schema`), model structs (`XxxRow` for reading, `NewXxx` for inserting, `XxxChangeset` for updating), PG enum mappings via `diesel-derive-enum` (`UserRoleMapping`, `ProjectViewTypeMapping`, `ProjectStatusMapping`, `RecurrenceTypeMapping`), `DbPool` type alias for `diesel_async::deadpool::Pool<AsyncPgConnection>`.
...
- **`stores`** — Reactive client state (`north-stores`). `AppStore` wraps `TaskStore` + `ProjectStore` + `TagStore` + `SavedFilterStore` + `TaskDetailModalStore` + `FilterDslStore` + `StatusBarStore` + `ModalStore` + `BrowserStorageStore`, provided globally via context. `TaskStore` holds tasks in `RwSignal<Vec<Task>>` — pages create filtered `Memo`s over the shared data. Supports optimistic updates (immediate UI, async sync). `FilterDslStore` manages DSL query text, async server-side validation, autocomplete suggestions, and execution results — the filter page controller delegates all DSL state to this store. `ModalStore` is a string-based modal registry (`open()`, `close()`, `is_open()`, `is_any_open()`) that decouples keyboard logic from DOM structure. `StatusBarStore` manages bottom-bar messages with info/danger variants. `BrowserStorageStore` is a reactive proxy over localStorage — lazily creates an `RwSignal<bool>` per key initialized from localStorage, writes update both the signal and localStorage (gated behind `hydrate` feature via `web-sys`). Individual stores (`TagStore`, `SavedFilterStore`) cache domain data for pickers and navigation.
...
- **`app`** — Leptos library crate. Features: `hydrate` (WASM client), `ssr` (server-side, pulls in north-core/north-server-fns/argon2/jsonwebtoken). Pages follow container/controller/view pattern and interact with stores for data. Complex stateful domain components live in `containers/` (pickers, sidebar, autocomplete, task checkbox, traversable task list, task list item, inline task input, detail modal, task meta). Simpler/presentational components live in `components/` (date picker, layout, filter autocomplete, recurrence modal, keybindings modal, status bar, drag_drop, theme toggle). **`atoms/`** contains UI Kit atoms — currently `Text` (TextVariant, TextColor, TextTag) based on Material Design 3 type scale. Atoms use `enum.classes() -> &'static str` pattern for variant/color/size mapping. Prefer `<Text>` over raw Tailwind for typography.
...
- **Container/controller/view pattern:** Pages with state management use a three-file pattern: `container.rs` (component entry, wires controller to view via inline `Callback` props), `controller.rs` (business logic, data loading, store interaction), `view.rs` (pure rendering). Simpler components use two-file container/view. Pure presentational components stay as single files. Callbacks are inlined directly into view props — no intermediate variables. Picker components (date, project, tag) support `icon_only` prop for compact action bar rendering in task cards. **Views must never access stores directly** — all store interactions go through the controller, which acts as a view-model. Views receive data (Memo/Signal/RwSignal) and callbacks from the controller; they only own pure UI state (toggles, input refs, local visibility flags).
- **Sub-components:** When a view defines `#[component]` sub-components inline, extract them into a `components/` subdirectory within the component's directory (`components/mod.rs` re-exports, one file per sub-component). The view imports via `use super::components::{...}`.
- **Containers vs components:** `containers/` holds complex stateful domain components that wire together stores, repositories, and rich interactions (pickers, sidebar, autocomplete, task checkbox, traversable task list, task list item, inline task input, detail modal, task meta). `components/` holds simpler or more presentational components (date picker, layout, filter autocomplete, recurrence modal, keybindings modal, status bar, drag_drop, theme toggle).
- **Atoms:** `atoms/` contains UI Kit atom components — semantic, multi-dimension prop components based on Material Design 3 type scale. Currently only `Text` (TextVariant, TextColor, TextTag). Each atom enum prop has `fn classes(self) -> &'static str` mapping Tailwind classes. Variant never includes color — color is always a separate prop. Each variant declares `default_tag()` for HTML element, overridable via `tag` prop. Prefer `<Text variant=TextVariant::HeadingLg>` over raw `<h1 class="text-2xl font-semibold ...">`. See `docs/UI_KIT.md` for the full component catalog.
...
- **Completed tasks toggle:** Pages pass `show_completed: RwSignal<bool>` to `TraversableTaskList` — the tree flattener includes completed tasks at the bottom of each parent group when toggled on.
- **TraversableTaskList:** Keyboard-driven tree list container (`containers/traversable_task_list/`). The sole list component for all pages. Builds a flat `Vec<FlatNode>` from hierarchical tasks via DFS pre-order (active sorted by sort_key first, completed at bottom per parent group). Maintains a task-ID-based cursor with tree-aware navigation (Up/Down = siblings, Left = parent, Right = first child). Supports inline editing (Enter to edit title, Enter to save, Escape to cancel) and inline creation (Ctrl+Enter after, Shift+Enter before, with Tab/Shift+Tab for indent/outdent). Exposes `TtlHandle` via optional `handle` prop — page views use it to trigger `start_create_top()` from "+ Add task" buttons, which opens an inline input at the top of the list. After creating the first task via `CreateTop`, the input chains below it as `InlineMode::Create { After }` for rapid sequential creation. Supports both keyboard reordering (Shift+Arrow) and drag-and-drop — when `item_config.draggable=true`, provides `DragDropContext` and handles tree-aware drops (Above/Below/Nest zones) with cycle prevention via `is_descendant_of()`. Two keyboard modes: **global** (default, uses `window_event_listener`, skips when ModalStore reports any modal open) and **scoped** (element-level `on:keydown`, used inside modals like TaskDetailModal to avoid conflicts with parent keyboard listeners). Auto-selects first task on Tab focus into scoped TTL. Rendered via `<For each=flat_nodes key=|n| n.task_id>` for efficient keyed updates. Each item reuses `TaskListItem` with `ItemConfig` passed through. Pure tree utilities in `tree.rs` with unit tests. `CreateTask` DTO supports optional `sort_key` for positional insertion via `sort_key_between()`/`sort_key_after()`. R key marks selected task as reviewed (when `item_config.show_review` is true).
- **ExtraVisibleIds:** Context type (`ExtraVisibleIds(RwSignal<Vec<i64>>)`) provided by `TraversableTaskList` container and `TaskDetailModal`. Keeps inline-created subtasks visible even when they exceed the sequential_limit. Scoped to the container's lifecycle — clears automatically on page navigation or modal close.
- **ItemConfig:** Config struct (`containers/task_list_item/container.rs`) bundling display/behavior props for `TaskListItem`: `show_review`, `show_project`, `draggable`. Defaults: `show_review: false`, `show_project: true`, `draggable: false`. Pages create an `ItemConfig` and pass it through `TraversableTaskList` → `TaskListItem`.
...
- **Recurrence:** Tasks support recurring schedules via `recurrence_type` (Scheduled = fixed schedule, AfterCompletion = relative to completion) and `recurrence_rule` (RRULE-like string e.g. `FREQ=WEEKLY;INTERVAL=1;BYDAY=MO,WE,FR`). Pure RRULE types live in `north-recurrence` crate (re-exported by `north-dto`). `RecurrenceRule` provides `parse()`/`to_rrule_string()` for serialization and `summarize()` for human-readable display. `RecurrenceModal` (`components/recurrence_modal/`) provides the UI — `ReactiveRecurrenceRule` wraps the pure `RecurrenceRule` with Leptos signals, constructed via `from_str()` factory and serialized back via `to_result()`. `TaskStore.set_recurrence()` handles async persistence.
- **Actionable visibility toggle:** Each page controller manages a `hide_non_actionable: Signal<bool>` backed by `BrowserStorageStore` with per-page keys (`north:hide-non-actionable:{page}`). Controllers expose `toggle_actionable_visibility()`. Views render an inline toggle button. The signal is passed as `Option<Signal<bool>>` to `TraversableTaskList`, which filters non-actionable tasks in the tree flattener.
- **User timezone:** `UserSettings.timezone` (default "UTC") stored in user settings JSONB. Settings page renders a grouped timezone dropdown from `constants::TIMEZONE_GROUPS`. Used by `RecurrenceController` to display recurrence times in the user's timezone.
