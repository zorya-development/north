...
impl TaskListItemController {
    ...
    pub fn delete(&self) {
        ...
        tasks.delete_task(self.task_id);
    }
    ...
    pub fn clear_start_at(&self) {
        let AppStore { tasks, .. } = self.app_store;
        tasks.clear_start_at(self.task_id);
    }

    pub fn set_project(&self, project_id: i64) {
        let AppStore { tasks, .. } = self.app_store;
        tasks.set_project(self.task_id, project_id);
    }

    pub fn clear_project(&self) {
        let AppStore { tasks, .. } = self.app_store;
        tasks.clear_project(self.task_id);
    }

    pub fn set_tags(&self, tag_names: Vec<String>) {
        let AppStore { tasks, .. } = self.app_store;
        tasks.set_tags(self.task_id, tag_names);
    }

    pub fn review(&self) {
        let AppStore { tasks, .. } = self.app_store;
        tasks.review_task(self.task_id);
    }
}
